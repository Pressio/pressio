...
TEST(containers_multi_vector_sharedmem_kokkos, staticCheck)
{
  ...
  {
    using v2_t = Kokkos::View<double**, Kokkos::LayoutLeft>;
    using mvec_t = containers::MultiVector<v2_t>;
    static_assert( containers::predicates::is_multi_vector_wrapper_kokkos<mvec_t>::value, "" );
    static_assert( !containers::predicates::is_vector_wrapper_kokkos<mvec_t>::value, "" );
  }
...
TEST(containers_multi_vector_sharedmem_kokkos, MustBe2dView)
{
  using namespace pressio;
  using view_type2 = Kokkos::View<double*>;
  static_assert( !containers::predicates::is_admissible_as_multi_vector_kokkos<view_type2>::value, "" );
}

TEST(containers_multi_vector_sharedmem_kokkos, MustBeLayoutLeft)
{
  using namespace pressio;
  using view_type = Kokkos::View<double**, Kokkos::LayoutLeft>;
  static_assert( containers::predicates::is_admissible_as_multi_vector_kokkos<view_type>::value, "" );

  using view_type2 = Kokkos::View<double**, Kokkos::LayoutRight>;
  static_assert( !containers::predicates::is_admissible_as_multi_vector_kokkos<view_type2>::value, "" );
}
