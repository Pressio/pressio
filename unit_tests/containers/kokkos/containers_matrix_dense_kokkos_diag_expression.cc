...
{
  {
    const auto diagO = pressio::containers::diag(A);
    EXPECT_EQ( diagO.extent(), 4 );
    EXPECT_DOUBLE_EQ( diagO(0), 1.2 );
    EXPECT_DOUBLE_EQ( diagO(1), 6.2 );
    EXPECT_DOUBLE_EQ( diagO(2), 11.2 );
  }
}
...
void testConst(const T & A){
  const  auto diagvals = pressio::containers::diag(A);
  EXPECT_EQ( diagvals.extent(), 4 );
  EXPECT_DOUBLE_EQ( diagvals(0), 44. );
  EXPECT_DOUBLE_EQ( diagvals(1), 6. );
  EXPECT_DOUBLE_EQ( diagvals(2), 11.2 );
