...
namespace{
  ...
  void doDot(const myMV_t & A, const myMV_t & B, matrix_t & C)
  {
    ...
    pressio::ops::product(::pressio::transpose(), pressio::nontranspose(), 1., A, B, 0., block);
...
TEST(ops_multi_vector_eigen, mv_dot_mvEigen_ColMajorMatrix){
  Data d;
...
TEST(ops_multi_vector_eigen, mv_dot_mvEigen_RowMajorMatrix){
  Data d;
...
TEST(ops_multi_vector_eigen, mv_dot_mvEigen_ColMajorMatrix_subspanStore){
  Data d;
  // matrix with left layout
  using eig_mat_t = Eigen::Matrix<double, -1, -1, Eigen::ColMajor>;
  using mat_t = ::pressio::containers::DenseMatrix<eig_mat_t>;
