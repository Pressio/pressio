...
public:
  ...
  state_type getGaussianICFull(const scalar_type mu) const
  {
	...
	U0(sid) = 1. + mu_ic_ * exp(-(pow(xGrid_(index_mapper(i, j)) - 1.5, 2) + pow(yGrid_(index_mapper(i, j)) - 1.5, 2)));
	U0(sid + 1) = 0.;
	U0(sid + 2) = 0.;
      }
    }
  ...
  state_type getGaussianIC(const scalar_type mu) const
  {
    ...
    for(int smpsIndexCounter = 0; smpsIndexCounter < nDofsStencil_; smpsIndexCounter++) {
      ...
      U0(indexStencil) = 1. + mu_ic_ * exp(-(pow(xGrid_(index_mapper(i, j)) - 1.5, 2) + pow(yGrid_(index_mapper(i, j)) - 1.5, 2)));
      U0(indexStencil + 1) = 0.;
      U0(indexStencil + 2) = 0.;
    }
    return U0;
