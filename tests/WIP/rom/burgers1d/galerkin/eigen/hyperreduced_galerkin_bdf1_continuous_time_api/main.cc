...
//!!!!!!!!!!
// this should not be taken as example, just using for testing
//!!!!!!!!!
struct MyDummyProjector
{
  decoder_jac_t phi_;

  MyDummyProjector(decoder_jac_t & phi) : phi_(phi){}

  template<class galerkin_rhs_t>
  void apply(const fom_velo_t & operand, galerkin_rhs_t & R) const
  {
    for (auto i=0; i<R.extent(0); ++i){
      R(i) = phi_.data()->col(i).dot(*operand.data());
    }
  }

  template<class galerkin_jacobian_t>
  void apply(const decoder_jac_t & operand, galerkin_jacobian_t & J) const
  {
    (*J.data()) = phi_.data()->transpose() * (*operand.data());
  }
};
