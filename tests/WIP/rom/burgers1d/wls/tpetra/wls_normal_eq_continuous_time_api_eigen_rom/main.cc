...
int main(int argc, char *argv[])
{
  ...
  using phi_native_mat_t = Tpetra::MultiVector<>;
  ...
  Tpetra::ScopeGuard tpetraScope (&argc, &argv);
  {
    ...
    const auto checkStr1 = pressio::testing::wls::doRun
      < fom_t, rom_data_t_eigen, tcomm_t, lowTri, phi_native_mat_t, ode_tag_euler>(Comm, rank);
    const auto checkStr2 = pressio::testing::wls::doRun
      < fom_t, rom_data_t_eigen, tcomm_t, lowTri, phi_native_mat_t, ode_tag_BDF2>(Comm, rank);
