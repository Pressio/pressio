...
struct EpetraExpDataFitN5
{
  ...
  shptr<state_type> tt_    = {}; // store the t value for the data
  shptr<state_type> yy_    = {}; // store the y values
  ...
  EpetraExpDataFitN5(){
    ...
    tt_ = std::make_shared<state_type>(*rowMap_);
    yy_ = std::make_shared<state_type>(*rowMap_);
    storeTimes();
    storeYValues();
  }//setUp
