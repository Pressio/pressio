...
int main(int argc, char *argv[]){
  ...
  // the lspg state is a wrapper of Kokkos vector, use layout left for now
  // so when exe space is the cpu, we access blas/lapack easily
  using kll = Kokkos::LayoutLeft;
  using k1dLl_d = Kokkos::View<scalar_t*, kll, exe_space>;
