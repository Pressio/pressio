...
#ifndef PRESSIO_TEST_ROM_LSPG_UNSTEADY_PREC_DEFAULT_CORRECTNESS_CHECKER_HPP_
#define PRESSIO_TEST_ROM_LSPG_UNSTEADY_PREC_DEFAULT_CORRECTNESS_CHECKER_HPP_
...
struct FakeNonLinSolver
{
  ...
  void solve(const SystemType & system, StateType & romState)
  {
    ...
    // note below the +1.5, which is here because that
    // is what the preconditiner does
	...
	for (int i=0; i<N_; ++i){
	  EXPECT_DOUBLE_EQ(R(i), -dt_*f(i)+1.5);
	}
	int count = 0;
	for (int i=0; i<N_; ++i){
	  for (int j=0; j<3; ++j){
	    const auto phi_val =  (double)count++;
	    EXPECT_DOUBLE_EQ(J(i,j), phi_val - dt_*(phi_val+mytime) + 1.5);
	  }
	}
	...
	for (int i=0; i<N_; ++i){
	  EXPECT_DOUBLE_EQ(R(i), ynp1(i)-yn(i)-dt_*f(i) + 1.5);
	}
	int count = 0;
	for (int i=0; i<N_; ++i){
	  for (int j=0; j<3; ++j){
