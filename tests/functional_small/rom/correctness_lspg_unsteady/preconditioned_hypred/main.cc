...
#define COMMON_PART1()\
  ...
  \
  fom_t fomSystem(sample_indices);\
...
TEST(rom_lspg, cont_time_unsteady_prec_hyperreduced_correctness_eigen)
{
  ...
  auto problem = plspg::create_prec_hyperreduced_unsteady_problem
    (pressio::ode::StepScheme::BDF1, fomSystem, decoder, romState, fomReferenceState, comb, prec);

  COMMON_PART2();
  pressio::log::finalize();
}

TEST(rom_lspg, cont_time_unsteady_prec_hyperreduced_correctness_custom_types)
{
  pressio::log::initialize(pressio::logto::terminal);
  pressio::log::setVerbosity({pressio::log::level::debug});
...
TEST(rom_lspg, disc_time_unsteady_prec_hyperreduced_correctness_eigen)
{
  pressio::log::initialize(pressio::logto::terminal);
  pressio::log::setVerbosity({pressio::log::level::debug});
  ...
  PreconditionerEigen prec;
  auto problem = pressio::rom::lspg::create_prec_hyperreduced_unsteady_problem<2>
...
TEST(rom_lspg, disc_time_unsteady_hyperreduced_correctness_custom_types)
{
  pressio::log::initialize(pressio::logto::terminal);
  pressio::log::setVerbosity({pressio::log::level::debug});
