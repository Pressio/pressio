...
public:
   ...
   const state_type & y_nm1) const
  {
    ...
    if (__this_test_failureHappended){
      ...
      const auto romStateStdVec_nm1 = romStateAtStepStarting.at(stepId-1);
      auto romState_nm1 = stdvec_to_eigen(romStateStdVec_nm1);
      Eigen::VectorXd tmp2 = phi * romState_nm1;
      EXPECT_TRUE( y_nm1.isApprox(tmp2) );
    }
...
TEST(rom_lspg_unsteady, fully_discrete_with_recovery_n3)
{
  ...
  */
  ...
  auto problem = pressio::rom::lspg::create_unsteady_problem<3>(space, fomSystem);
