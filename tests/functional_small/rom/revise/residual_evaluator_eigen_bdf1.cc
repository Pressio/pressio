...
struct RomStateProducerBdf1
{
  ...
  RomStateProducerBdf1(int romSize)
  {
    // times at n
    times_ = {10., 12., 15};
    M_.resize(romSize, 6);
...
struct ObserverBdf1
{
  ...
  void operator()(std::size_t instanceId, const T & R)
  {
    ...
    if (instanceId==2){
      ...
      fomObj_.velocity(yn, timen, f_);
      R_gold_ = yn - ynm1 - 0.3*f_;
      EXPECT_TRUE(R_gold_.isApprox(R));
    }
  }
};

TEST(rom, residual_evaluator_bdf1)
{
