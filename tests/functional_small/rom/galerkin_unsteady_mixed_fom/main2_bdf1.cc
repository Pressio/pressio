...
namespace galex = pressio::rom::galerkin::experimental;
...
struct MyFom
{
  ...
  void rhsAndJacobian(const state_type & yIn,
		      independent_variable_type timeIn,
		      rhs_type & R,
	   ...
	   rhs_type & r) const
  {
    ...
    pressio::ops::deep_copy(r, yIn);
    for (int i=0; i<r.size(); ++i){ r(i) += timeIn; }
  }
...
struct MyFakeNonLinSolverForFOM{
  ...
  void solve(const SystemType & system, StateType & state)
  {
    ...
    r_t goldR1(_m); goldR1.setConstant(-24.);
    r_t goldR2(_m); goldR2.setConstant(-25.);
    r_t goldR5(_m); goldR5.setConstant(-2724.);
    r_t goldR6(_m); goldR6.setConstant(-2725.);
...
struct MyFakeNonLinSolverForROM{
  ...
  void solve(const SystemType & system, StateType & state)
  {
    ...
    using r_t = typename SystemType::residual_type;
    r_t goldR3(_n); goldR3 << -21568., -43136., -64704.;
    r_t goldR4(_n); goldR4 << -21663., -43327., -64991.;
...
struct Subdomain
{
  ...
  using mixed_problem_t =
    decltype(galex::create_unsteady_implicit_problem_mixed_fom(odeScheme,
  ...
  Subdomain(const vec_t & shift, const phi_t & phiIn)
    : fomObj_(), fomState_(_m), romState_(_n),
      ...
      mixedProblem_(galex::create_unsteady_implicit_problem_mixed_fom(odeScheme,
								      romSubspace_,
								      fomObj_)),
      fomNonLinSolver_(),
      romNonLinSolver_()
...
TEST(rom_galerkin_unsteady_implicit, test_bdf1_fom_rom_fom)
{
  /*
  stepID     1          2          3
       o -------- o -------- o -------- o
  ...
  */
  ...
  const int numFomSteps = 2;
  const int numRomSteps = 1;
  ...
  pressio::ops::fill(goldFomState, 1358.);
  ASSERT_TRUE( goldFomState.isApprox(computedFomState) );
  Eigen::VectorXd goldRomState(_n);
  goldRomState[0] = 98.; goldRomState[1] = 194.; goldRomState[2] = 290.;
