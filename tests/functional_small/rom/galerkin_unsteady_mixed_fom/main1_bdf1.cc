...
struct MyFakeNonLinSolverForFOM{
  ...
  void solve(const SystemType & system, StateType & state)
  {
    ...
    r_t goldR3(_m); goldR3.setConstant(-56.);
    r_t goldR4(_m); goldR4.setConstant(-57.);
...
struct MyFakeNonLinSolverForROM{
  ...
  void solve(const SystemType & system, StateType & state)
  {
    ...
    using r_t = typename SystemType::residual_type;
    r_t goldR1(_n); goldR1 << -224., -448., -672.;
    r_t goldR2(_n); goldR2 << -319., -639., -959.;
    r_t goldR5(_n); goldR5 << -46688., -93376., -140064.;
    r_t goldR6(_n); goldR6 << -46783., -93567., -140351.;
...
TEST(rom_galerkin_unsteady_implicit, test_bdf1_rom_fom_rom)
{
  /*
  stepID     1          2          3
       o -------- o -------- o -------- o
