...
constexpr int _m = 8;
constexpr int _n = 3;
constexpr pode::StepScheme odeScheme = pode::StepScheme::BDF2;
...
struct MyFakeNonLinSolverForFOM{
  ...
  void solve(const SystemType & system, StateType & state)
  {
    ...
    r_t goldR2(_m); goldR2.setConstant(-1.);
    r_t goldR3(_m); goldR3.setConstant(-2.);
    r_t goldR4(_m); goldR4.setConstant(-1.);
    r_t goldR9(_m); goldR9.setConstant(-16./3.);
    r_t goldR10(_m); goldR10.setConstant(-13./3.);
...
struct MyFakeNonLinSolverForROM{
  ...
  void solve(const SystemType & system, StateType & state)
  {
    ...
    using r_t = typename SystemType::residual_type;
    r_t goldR5(_m); goldR5.setConstant(-76.);
    r_t goldR6(_m); goldR6.setConstant(-70.);
    r_t goldR7(_m); goldR7.setConstant(-16/3.);
    r_t goldR8(_m); goldR8.setConstant(2./3.);
...
struct Subdomain
{
  ...
  void doStep(const double startTime, const int step, const double dtIn)
  {
    ...
    if      (step == 1){ ASSERT_DOUBLE_EQ(startTime, 0.); }
    else if (step == 2){ ASSERT_DOUBLE_EQ(startTime, dtIn); }
    else if (step == 3){ ASSERT_DOUBLE_EQ(startTime, 2*dtIn); }
    else if (step == 4){ ASSERT_DOUBLE_EQ(startTime, 3*dtIn); }
    else if (step == 5){ ASSERT_DOUBLE_EQ(startTime, 4*dtIn); }

    write_vec_cout("before step, fomState = ", fomState_); std::cout << '\n';
    write_vec_cout("before step, romState = ", romState_); std::cout << '\n';

    if (step <= 2){
      mixedProblem_(prom::FomStep, fomState_, startAt, stepNumber, dt, fomNonLinSolver_);
    }
    else if (step == 3){
  ...
  const auto & fomState() const{ return fomState_; }
  const auto & romState() const{ return romState_; }
...
TEST(rom_lspg_unsteady, test_bdf2_fom_fom_rom_rom_fom)
{
  /*
  stepID     1          2          3          4          5
  ...
  */
  ...
  auto goldFomState = pressio::ops::clone(computedFomState);
  pressio::ops::fill(goldFomState, 1594.);
  ASSERT_TRUE( goldFomState.isApprox(computedFomState) );

  auto goldRomState = pressio::ops::clone(computedRomState);
  goldRomState[0] = 116.; goldRomState[1] = 228.; goldRomState[2] = 340.;
  ASSERT_TRUE( goldRomState.isApprox(computedRomState) );
