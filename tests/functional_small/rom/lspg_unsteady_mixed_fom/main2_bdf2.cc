...
struct MyFakeNonLinSolverForFOM{
  ...
  void solve(const SystemType & system, StateType & state)
  {
    ...
    r_t goldR5(_m); goldR5.setConstant(-16./3.);
    r_t goldR6(_m); goldR6.setConstant(-13./3.);
...
struct MyFakeNonLinSolverForROM{
  ...
  void solve(const SystemType & system, StateType & state)
  {
    ...
    using r_t = typename SystemType::residual_type;
    r_t goldR3(_m); goldR3.setConstant(-76.);
    r_t goldR4(_m); goldR4.setConstant(-70.);
...
TEST(rom_lspg_unsteady, test_bdf2_fom_rom_fom)
{
  /*
  stepID     1          2          3
       o -------- o -------- o -------- o
