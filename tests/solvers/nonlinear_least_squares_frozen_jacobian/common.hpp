...
// : pressio::solvers::LinearBase<mat_type, MyLinSolverNormalEq>
{
	     ...
	     state_t & correction)
  {
    ...
    if(iterCount_ == 3) {
      if(!H.data()->isApprox(trueH3))
	checkStr_ = "FAILED";
      if(!g.data()->isApprox(trueG3))
	checkStr_ = "FAILED";
    }

    if(iterCount_ == 4) {
      if(!H.data()->isApprox(trueH4))
	checkStr_ = "FAILED";
      if(!g.data()->isApprox(trueG4))
	checkStr_ = "FAILED";
    }

    if(iterCount_ == 5) {
      if(!H.data()->isApprox(trueH5))
	checkStr_ = "FAILED";
      if(!g.data()->isApprox(trueG5))
	checkStr_ = "FAILED";
    }

    if(iterCount_ == 6) {
      if(!H.data()->isApprox(trueH6))
	checkStr_ = "FAILED";
      if(!g.data()->isApprox(trueG6))
	checkStr_ = "FAILED";
    }
  }
};
...
struct MySolverQR
{
  ...
  void computeThin(const J_t & Jac)
  {
    ...
    if(iterCount_ == 1) {
      if(!Jac.data()->isApprox(trueJ1))
	checkStr_ = "FAILED";
    }

    if(iterCount_ == 2) {
      if(!Jac.data()->isApprox(trueJ2))
	checkStr_ = "FAILED";
    }

    if(iterCount_ == 3) {
      if(!Jac.data()->isApprox(trueJ3))
	checkStr_ = "FAILED";
    }

    if(iterCount_ == 4) {
      if(!Jac.data()->isApprox(trueJ4))
	checkStr_ = "FAILED";
    }

    if(iterCount_ == 5) {
      if(!Jac.data()->isApprox(trueJ5))
	checkStr_ = "FAILED";
    }

    if(iterCount_ == 6) {
      if(!Jac.data()->isApprox(trueJ6))
	checkStr_ = "FAILED";
    }
  }
};
