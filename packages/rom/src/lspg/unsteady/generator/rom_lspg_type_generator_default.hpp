...
template <
  ...
  typename time_discrete_ud_ops
  >
struct DefaultLSPGTypeGenerator
  : LSPGCommonTypes<
  fom_type, decoder_type, lspg_state_type, odeName, time_discrete_ud_ops
  >{

  using base_t = LSPGCommonTypes
    <fom_type, decoder_type, lspg_state_type,
     odeName, time_discrete_ud_ops>;
  ...
  using typename base_t::td_ud_ops;
  ...
  using lspg_residual_policy_t	= ::rompp::rom::LSPGResidualPolicy<
    fom_states_data, fom_rhs_data, fom_eval_rhs_policy_t, td_ud_ops>;

  // policy defining how to compute the LSPG time-discrete jacobian
  using lspg_jacobian_policy_t	= ::rompp::rom::LSPGJacobianPolicy<
    fom_states_data, lspg_matrix_t, fom_apply_jac_policy_t, decoder_t, td_ud_ops>;
