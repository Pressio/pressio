...
private:
  ...
  using wls_jacs_t = std::vector<jac_t>; // currently have this as a vector of jacobians, can change to container
...
public:
  ...
  odeObj_.updateStatesFirstStep(wlsStateIC,fomStateReconstrObj_,odeObj_.auxStatesContainer_); // reconstruct the FOM states from the previous window/ICs 

  odeObj_.time_discrete_residual(appObj,yFOM_current_,residual_,odeObj_.auxStatesContainer_,ts,dt,step); //compute the time discrete residual
  ...
  for (int i = 0; i < time_stencil_size; i++){
    odeObj_.time_discrete_jacobian(appObj,yFOM_current_,wlsJacs_[time_stencil_size - i - 1],phi_,odeObj_.auxStatesContainer_,n*dt,dt,step,i); //compute the Jacobian for each state in the stencil
  }
  ...
  for (int n = 1; n < numStepsInWindow; n++){
    ...
    odeObj_.updateStatesNStep(yFOM_current_,odeObj_.auxStatesContainer_); 
    ...
    t = ts + n*dt;
    step = step_s + n; 
    odeObj_.time_discrete_residual(appObj,yFOM_current_,residual_,odeObj_.auxStatesContainer_,t,dt,step);
  ...
  fomStateReconstrObj_(wlsCurrentState,yFOM_current_);
  odeObj_.updateStatesFirstStep(wlsStateIC,fomStateReconstrObj_,odeObj_.auxStatesContainer_);
  odeObj_.time_discrete_residual(appObj,yFOM_current_,residual_,odeObj_.auxStatesContainer_,ts,dt,step);
  rnorm += ::pressio::containers::ops::norm2(residual_);
  for (int n = 1; n < numStepsInWindow; n++){
    // === reconstruct FOM states ========
    odeObj_.updateStatesNStep(yFOM_current_,odeObj_.auxStatesContainer_); 
