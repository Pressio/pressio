...
int main(int argc, char *argv[]){
  ...
  Tpetra::ScopeGuard tpetraScope (&argc, &argv);
  {
    ...
    using lspg_residual_t = typename lspg_problem_types::lspg_residual_t;
    ...
    //   scalar_t, qr_type, converged_when_t, rom_stepper_t>;//void, lspg_state_t, lspg_residual_t, rom_jac_t>;
    // gnsolver_t solver(stGen.stepperObj_, yROM);
    // solver.setTolerance(1e-14);
    // solver.setMaxIterations(100);
    ...
    int shift = 0;
    if (rank==1)  shift = 25;
    int myn = 25;//yRf.getDataMap().NumMyElements();
    for (auto i=0; i<myn; i++){
      std::cout << i << " " << yRf_v[i] << std::endl;
      //assert(std::abs(yRf_v[i] - bdf1Sol[i+shift]) < 1e-12 );
     }
  }
