...
TEST(core_ops_eigen, sparseMatProdMultivector){
  using namespace rompp;
  
  using esp_t = Eigen::SparseMatrix<double,Eigen::RowMajor,int>;
  using mymat_t = core::Matrix<esp_t>;
  ...
  using myMV_t = core::MultiVector<Eigen::MatrixXd>;
  myMV_t mv(4,3);
  ...
  auto c1 = rompp::core::ops::product(A,mv);
  ...
  rompp::core::Matrix<Eigen::MatrixXd> c(6,3);
  rompp::core::ops::product(A,mv,c);
