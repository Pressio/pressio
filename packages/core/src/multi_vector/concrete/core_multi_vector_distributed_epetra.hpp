...
{
private:
  using derived_t = MultiVector<wrapped_type>;
...
public:
  MultiVector() = delete;
  explicit MultiVector(const map_t & mapobj) : data_(mapobj){}
  explicit MultiVector(const wrap_t & vecobj) : data_(vecobj){}
  ~MultiVector() = default;

  
private:
  friend ContainerBase< derived_t, wrapped_type >;
  friend MultiVectorDistributedBase< derived_t >;
  friend MultiVectorDistributedTrilinos< derived_t >;
  friend MultiVectorMathBase< derived_t >;
