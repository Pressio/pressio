...
void readMaskGIDs(std::string filename,
		  std::vector<int> & A){
...
int main(int argc, char *argv[]){
  ...
  {
    //read GIDs for mask
    std::vector<int> rGIDs;
    readMaskGIDs("r_gids.dat", rGIDs);
    ...
    // define LSPG problem
    using lspg_problem_type = pressio::rom::MaskedLSPGTypeGenerator<
    ...
    // integrate in time
    pressio::ode::integrateNSteps(lspgProblem.stepperObj_, yROM, t0, dt, 3, solver);
