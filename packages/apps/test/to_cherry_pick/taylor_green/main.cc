...
void proc_kToij(int & iin, int & jin, int N, int k){
  jin = proc_kToj(N,k);
  iin = proc_kToi(N,k);
}
...
{  
  ...
  for (int li=0; li<lN_; li++){
    for (int lj=0; lj<lN_; lj++)
      {
	int thisGID = MyGlobalElements[localCount]; // global ID of current point
	...
	if (thisGID != 0 ){
	  ...
	  westID = (gj==0) ? grid_locijToGlobk(neighbors.at(neighTag::left), li, lN_-1, lN_) : (lj==0) ? thisGID-lNsq_+lN_-1 : thisGID-1;
	  eastID = (gj==gN_-1) ? grid_locijToGlobk(neighbors.at(neighTag::right), li, 0, lN_) : (lj==lN_-1) ? thisGID+lNsq_-lN_+1 : thisGID+1;

	  northID = (gi==0) ? grid_locijToGlobk(neighbors.at(neighTag::top), lN_-1, lj, lN_) :
	    (li==0) ? grid_locijToGlobk(neighbors.at(neighTag::top), lN_-1, lj, lN_) : thisGID-lN_;
	  southID = (gi==gN_-1) ? grid_locijToGlobk(neighbors.at(neighTag::bottom), 0, lj, lN_) :
	    (li==lN_-1) ? grid_locijToGlobk(neighbors.at(neighTag::bottom), 0, lj, lN_) : thisGID+lN_;
...
int main(int argc, char *argv[])
{
  ...
  // if(myID_==2){
  // std::cout << " gi = " << grid_globkToglobi(lN_, myID_, myIDi_, myIDj_, 86)
  // 	      << " gj = " << grid_globkToglobj(lN_, myID_, myIDi_, myIDj_, 86)
  // 	      << std::endl;
  // }
