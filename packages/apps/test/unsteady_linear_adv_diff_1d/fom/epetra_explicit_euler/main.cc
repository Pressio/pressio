...
int main(int argc, char *argv[]){
  ...
  //---------------------------------------------------------------------------
  // States and forcing term to calculate the residual
  //---------------------------------------------------------------------------
  const native_state y0n(*appObj.getInitialState());
  auto r0n = appObj.residual(y0n, static_cast<scalar_t>(0)); //Spatial residual
  //---------------------------------------------------------------------------
  ...
  y.data()->Print(std::cout <<std::setprecision(14));
  {
    ...
    checkSol(rank, y, 
	     UnsteadyLinAdvDiff1dExpGoldStates<ode_case>::get(mu, domain, bc1D, dt, 
							     fint));
  }
