...
#include "vector/algebra_vector_epetra.hpp"
#include "vector/algebra_vector_serial_arbitrary.hpp"
#include "vector/algebra_vector_std.hpp"
...
int main(int argc, char *argv[]){
  {
    ...
    using vec_t = algebra::Vector<state_t>;
    vec_t gigi;
    gigi.resize(10);
    gigi[0] = 22.4;
    state_t const * myview = gigi.view();
    std::cout << (*myview)[0] << " " << (*myview)[1] << std::endl;
    std::cout << algebra::details::traits<vec_t>::isSTDVector << std::endl;
  }
  ...
  {  
    ...
    using vec_t = algebra::Vector<Epetra_Vector>;
    vec_t gigi2(x);
