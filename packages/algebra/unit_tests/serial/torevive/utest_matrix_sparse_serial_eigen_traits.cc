...
template<typename T>
struct algebra_matrix_sparse_serial_eigen_traitsTest
  : public ::testing::Test{
public:
  ...
  using my_t = algebra::Matrix<native_t>;
  using myTrait = algebra::details::traits<my_t>;
...
typedef ::testing::Types< typesEig<int,Eigen::RowMajor,int>,
			  ...
			  > MyTypes;
TYPED_TEST_CASE(algebra_matrix_sparse_serial_eigen_traitsTest, MyTypes);

TYPED_TEST(algebra_matrix_sparse_serial_eigen_traitsTest, traits)
{
	using namespace rompp;
  ...
  using type1 = algebra::Matrix<Eigen::SparseMatrix<double,Eigen::RowMajor,int>>;  
  using type2 = algebra::Matrix<Eigen::SparseMatrix<double,Eigen::ColMajor,int>>;
  static_assert( algebra::meta::sparse_sharedmem_eigen_same_storage<type1,type2>::value == false, "ohh" );
}
