...
TEST(algebra_matrix_matrix_product, eigenDenseDense){
  using namespace rompp;

  using nat_t = Eigen::MatrixXd;
  using myA_t = algebra::Matrix<nat_t>;
  nat_t a(3,4);
  ...
  using myB_t = algebra::Matrix<nat2_t>;
  nat2_t b(4,2); b << 1.,2.,3.,3.,4.,4.,2.,2.;
  myB_t B(b);
  //  std::cout << *B.data();

  static_assert(
   algebra::meta::is_dense_matrix_wrapper_eigen<myA_t>::value,"");
  
  auto C = algebra::ops::product(A,B);
  EXPECT_EQ( C.cols(), B.cols());
  EXPECT_EQ( C.rows(), A.rows());
  EXPECT_DOUBLE_EQ( C(0,0), 27.0);
  EXPECT_DOUBLE_EQ( C(1,0), 23.0);

  algebra::Matrix<Eigen::Matrix<double,3,2>> C2;
  algebra::ops::product(A,B,C2);
...
TEST(algebra_matrix_matrix_product, eigenSparseSparse){  
  using namespace rompp;  
...
TEST(algebra_matrix_matrix_product, eigenSparseDense){
  using namespace rompp;  
  // create sparse matrix
  using mymat_t = algebra::Matrix<
    Eigen::SparseMatrix<double,Eigen::RowMajor,int>>;
  ...
  // create dense matrix
  using nat_t = Eigen::MatrixXd;
  using myB_t = algebra::Matrix<nat_t>;
  nat_t b(3,4);
  ...
  algebra::Matrix<Eigen::MatrixXd> C2(4,4);
  algebra::ops::product(A,B,C2);
  for (int i=0; i<4; i++)
    for (int j=0; j<4; j++)
