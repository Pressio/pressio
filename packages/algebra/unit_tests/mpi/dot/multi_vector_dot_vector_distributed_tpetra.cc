...
TEST_F(tpetraMultiVectorR9C4VecS9Fixture, MVVecDotProduct){
  ...
  using vec_t = algebra::Vector<nat_v_t>;
  using v_device_t = typename algebra::details::traits<vec_t>::device_t;
  ...
  // //MV dot b = c2
  algebra::Vector<Eigen::VectorXd> c2;
  c2.resize(4);
  algebra::ops::dot(MV, VV, c2);
  EXPECT_EQ( c2.size(), 4);
  ...
  algebra::Vector<natvec_t3> c3(4);
  algebra::ops::dot(MV, VV, c3);
  EXPECT_EQ( c3.size(), 4);
