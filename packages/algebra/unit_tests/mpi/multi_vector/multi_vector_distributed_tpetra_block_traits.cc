...
TEST(algebra_multi_vector_distributed_tpetra_block, Traits){
  ...
  using mymvec_t = algebra::MultiVector<nat_t>;
  ...
  STATIC_ASSERT_IS_ALGEBRA_MULTI_VECTOR_WRAPPER(mymvec_t);

  using mvecTrait = algebra::details::traits<mymvec_t>;
  ...
  ASSERT_TRUE(mvecTrait::wrapped_multi_vector_identifier
  	      == algebra::details::WrappedMultiVectorIdentifier::TpetraBlock);
