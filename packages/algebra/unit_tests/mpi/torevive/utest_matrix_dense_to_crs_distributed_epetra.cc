...
struct algebra_matrix_dense_to_crs_distributed_epetraFix
  : public ::testing::Test{
public:
  int rank_;
  ...
  // algebra::Matrix<Epetra_CrsMatrix> * As_;
  // Epetra_BlockMap * smMap_;
  algebra::Matrix<Epetra_MultiVector> * Ad_;
  ...
  virtual void SetUp(){
    ...
    // As_ = new algebra::Matrix<Epetra_CrsMatrix>(*smMap_, 5);
    // //------------------------------------------

    // setup dense and vector for test 2
    dmMap_ = new Epetra_Map(9, 0, *Comm_);
    Ad_ = new algebra::Matrix<Epetra_MultiVector>(*dmMap_, 5);
    //------------------------------------------    
  }
...
TEST_F(algebra_matrix_dense_to_crs_distributed_epetraFix, CRSMatTimesVector)
{
  {
    if (MyPID_ == 0){
      (*Ad_)(0,0) = 1.0;
  ...
  auto B = algebra::denseToSparse(*Ad_);
...
// TEST_F(algebra_matrix_vec_product_distributed_epetraFix, DenseMatTimesVector)
// {
...
//   // product: b = A b
//   //------------------
//   auto c = algebra::matrixVectorProduct( *Ad_, *b2_ );  
//   c.data()->Print(std::cout);
