...
public:
  ...
  algebra::Matrix<Epetra_CrsMatrix> * B_;
  ...
  virtual void SetUp()
  {
    ...
    B_ = new algebra::Matrix<Epetra_CrsMatrix>(*contigMap_, 4);
  }
...
TEST_F(algebra_matrix_sparse_distributed_epetraFix,Test1)
{
  //-----------
  // FILL A 
  //-----------
  ...
  //-----------
  {
    algebra::Matrix<Epetra_CrsMatrix> myC_(*contigMap_, 3);
    algebra::mat_ops::product(*A_, *B_, myC_, false, false);
  ...
  //-----------
  {
    auto myC_ = algebra::mat_ops::product(*A_, *B_, false, false);
    {
      double o_v[2]; int o_i[2]; int n;
      int ec = myC_.data()->ExtractGlobalRowCopy( 0, 2, n, o_v, o_i );
      if (MyPID_ == 0){
