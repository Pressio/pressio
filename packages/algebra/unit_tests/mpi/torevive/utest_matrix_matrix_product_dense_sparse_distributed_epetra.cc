...
struct algebra_matrix_dense_sparse_distributed_epetraFix
  : public ::testing::Test{
public:
  int rank_;
  ...
  algebra::Matrix<Epetra_MultiVector> * B_;
  ...
  virtual void SetUp()
  {
    ...
    A_ = new algebra::Matrix<Epetra_CrsMatrix>(*smMap_, 5);

    nRowsDM_ = 13; //A_->globalCols();
    nColsDM_ = 5;
    dmMap_ = new Epetra_Map(nRowsDM_, 0, *Comm_);
    dmMap_->Print(std::cout);
    B_ = new algebra::Matrix<Epetra_MultiVector>(*dmMap_, nColsDM_);
  }
...
TEST_F(algebra_matrix_dense_sparse_distributed_epetraFix, Test1)
{
  ...
  // // product: C = A B
  // //------------------
  // auto CC = algebra::mat_ops::product(*A_, *B_);
  // CC.data()->Print(std::cout);

  // assert( CC.globalRows() == 9);
  // assert( CC.globalCols() == 5);
  // static_assert( std::is_same<decltype(CC), algebra::Matrix<Epetra_MultiVector>>::value, "" );
  // if (MyPID_ == 0){
