...
public:
  ...
  using my_t = containers::Matrix<native_t>;
  using myTrait = containers::details::traits<my_t>;
...
typedef ::testing::Types< typesEig<int,Eigen::RowMajor,int>,
			  ...
			  > MyTypes;
TYPED_TEST_CASE(containers_matrix_sparse_serial_eigen_traitsTest, MyTypes);
...
TYPED_TEST(containers_matrix_sparse_serial_eigen_traitsTest, traits)
{
  ...
  using type1 = containers::Matrix<Eigen::SparseMatrix<double,Eigen::RowMajor,int>>;  
  using type2 = containers::Matrix<Eigen::SparseMatrix<double,Eigen::ColMajor,int>>;
  static_assert( containers::meta::sparse_sharedmem_eigen_same_storage<type1,type2>::value == false, "ohh" );
}
