...
public:
  ...
  using myvec_t = containers::Vector<stdV_t>;
  ...
  void check(){   
    ...
    ::testing::StaticAssertTypeEq<typename
				  vecTrait::ordinal_t,
				  containers::default_types::local_ordinal_t>();
    ::testing::StaticAssertTypeEq<typename
    ...
    ASSERT_TRUE(vecTrait::wrapped_vector_identifier 
      == containers::details::WrappedVectorIdentifier::CppStdLib);
    ASSERT_TRUE(vecTrait::is_shared_mem == 1);
  }
};

struct containers_vector_serial_stdlib_traits_Fixture : public ::testing::Test{
public:
  // virtual void SetUp(){}
...
TEST_F(containers_vector_serial_stdlib_traits_Fixture, traits)
{
  a.check();
  b.check();
  c.check();
