...
struct containers_matrix_sparse_distributed_epetraFix
  : public ::testing::Test{
public:
  int rank_;
  ...
  containers::Matrix<Epetra_CrsMatrix> * B_;
  ...
  virtual void SetUp()
  {
    ...
    B_ = new containers::Matrix<Epetra_CrsMatrix>(*contigMap_, 4);
  }
...
TEST_F(containers_matrix_sparse_distributed_epetraFix,Test1)
{
  //-----------
  // FILL A 
  //-----------
  ...
  //-----------
  {
    containers::Matrix<Epetra_CrsMatrix> myC_(*contigMap_, 3);
    containers::mat_ops::product(*A_, *B_, myC_, false, false);
  ...
  //-----------
  {
    auto myC_ = containers::mat_ops::product(*A_, *B_, false, false);
    {
      double o_v[2]; int o_i[2]; int n;
      int ec = myC_.data()->ExtractGlobalRowCopy( 0, 2, n, o_v, o_i );
      if (MyPID_ == 0){
