...
.. note::
    ...
    to improve this page, `open an issue on github <https://github.com/Pressio/pressio/issues>`_.
...
.. tip::
    ...
    An explicit Galerkin problem satisfies the `steppable concept <ode_advance.html>`_
    (specifically, behaves like an `explicit stepper <ode_steppers_explicit.html>`_\ ).
...
.. tip::
    ...
    Main thing to remember:
    An implicit Galerkin problem satisfies the `steppable concept <ode_advance.html>`_
    (specifically, behaves like an `implicit stepper <ode_steppers_implicit.html>`_\ ).
...
`For implicit, in fact, you also need a solver to compute the solution at the next step <ode_steppers_implicit.html>`_.
