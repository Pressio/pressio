...
=======
{\ttfamily pressio} is header-\/only, so to use it one does not need to precompile it into a library and linking to it. However, since we use preprocessor directives to conditionally enable/disable code based on target third-\/party libraries, one needs to account for this. See below for the details. \hypertarget{md_pages_getstarted_build_and_install_autotoc_md15}{}\doxysection{Dependencies}\label{md_pages_getstarted_build_and_install_autotoc_md15}
>>>>>>> testB
...
=======
There might be other constraints on the variables one can set. The reason for this is twofold\+: (a) to simplify what the user needs to provide; and (b) we belive some of these constraints are necessary, like the Trilinos example above or always requiring B\+L\+AS and L\+A\+P\+A\+CK to be simulateneously on.\hypertarget{md_pages_getstarted_build_and_install_autotoc_md16}{}\doxysection{In practice, what are the steps to get, install and use Pressio?}\label{md_pages_getstarted_build_and_install_autotoc_md16}
>>>>>>> testB
We suggest to follow these steps\+: 
\begin{DoxyEnumerate}
\item Clone \href{https://github.com/Pressio/pressio}{\texttt{ pressio}} (defaults to the master branch)
