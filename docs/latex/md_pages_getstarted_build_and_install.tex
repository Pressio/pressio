...
{\ttfamily pressio} is header-\/only, so to use it one does not need to precompile it into a library and linking to it. However, since we use preprocessor directives to conditionally enable/disable code based on target third-\/party libraries, one needs to account for this. See below for the details. \hypertarget{md_pages_getstarted_build_and_install_autotoc_md16}{}\doxysection{Dependencies}\label{md_pages_getstarted_build_and_install_autotoc_md16}
...
\hline
\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ T\+PL Library Name  }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ Optional/\+Required  }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ Version Known to Work   }\\\cline{1-3}
\endfirsthead
\hline
\endfoot
\hline
...
There might be other constraints on the variables one can set. The reason for this is twofold\+: (a) to simplify what the user needs to provide; and (b) we belive some of these constraints are necessary, like the Trilinos example above or always requiring B\+L\+AS and L\+A\+P\+A\+CK to be simulateneously on.\hypertarget{md_pages_getstarted_build_and_install_autotoc_md17}{}\doxysection{In practice, what are the steps to get, install and use Pressio?}\label{md_pages_getstarted_build_and_install_autotoc_md17}

\begin{DoxyEnumerate}
\item Clone \href{https://github.com/Pressio/pressio}{\texttt{ pressio}} (defaults to the master branch), or you can pick a \href{https://github.com/Pressio/pressio/releases}{\texttt{ release version}} 
...
We also remark that during the configuration step above pressio does not need to know where a target T\+PL exists in your system. In the configuration step above, you are simply telling Pressio that you have a certain T\+PL and want to enable the corresponding code in pressio. The T\+P\+Ls will be needed at linking stage when you build {\itshape your} code that {\itshape uses} pressio.
...
\begin{DoxyParagraph}{}
The procedure above is advised because it enables {\ttfamily pressio} to properly process the cmake options and turn on/off based on certain conditions (as explained above). The alternative way to use pressio would be to just clone the repo, point to the {\ttfamily $<$where-\/you-\/cloned-\/pressio$>$/packages} subdirectory and use cmake variables directly when building your code. However, this could have unexpected consequences since you would be resposible to set the variables correctly but you would not know exactly all the possible constraints.
\end{DoxyParagraph}
\hypertarget{md_pages_getstarted_build_and_install_autotoc_md18}{}\doxysection{Where to go from here?}\label{md_pages_getstarted_build_and_install_autotoc_md18}
