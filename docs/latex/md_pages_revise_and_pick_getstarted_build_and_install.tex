...
{\ttfamily pressio} is header-\/only, so to use it one does not need to precompile it into a library and linking to it. However, since we use preprocessor directives to conditionally enable/disable code based on target third-\/party libraries, one needs to account for this. See below for the details. \hypertarget{md_pages_revise_and_pick_getstarted_build_and_install_autotoc_md184}{}\doxysection{Dependencies}\label{md_pages_revise_and_pick_getstarted_build_and_install_autotoc_md184}
...
\end{DoxyParagraph}
\hypertarget{md_pages_revise_and_pick_getstarted_build_and_install_autotoc_md186}{}\doxysection{Where to go from here?}\label{md_pages_revise_and_pick_getstarted_build_and_install_autotoc_md186}
