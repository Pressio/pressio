...
\end{DoxyParagraph}
\hypertarget{md_pages_coreconcepts_adapter_api_autotoc_md2}{}\doxysection{Continuous-\/time A\+PI}\label{md_pages_coreconcepts_adapter_api_autotoc_md2}
This A\+PI is intended for any system expressible in {\itshape time-\/continuous} form as \[ \frac{d \boldsymbol{y}}{dt} = \boldsymbol{f}(\boldsymbol{y},t; \boldsymbol{\mu}), \quad \boldsymbol{y}(0;\boldsymbol{\mu}) = \boldsymbol{y}(\boldsymbol{\mu}), \] where $y$ is the full-\/order model (F\+OM) state, $f$ the F\+OM velocity, and $t$ is time. todo finish.
...
This A\+PI is intended for any system expressible in a discrete-\/time form as \[ \boldsymbol{R}(\boldsymbol{y}, \boldsymbol{y_{n-1}}, ..., t_n, dt_n; ...) = \boldsymbol{0} \] where $y$ is the full-\/order model (F\+OM) state, $t$ is time, and $R$ is the residual. todo finish.
