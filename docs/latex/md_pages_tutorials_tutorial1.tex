...
\begin{DoxyParagraph}{}
This tutorial shows how to create a linear decoder object for data structures already supported in pressio.
\end{DoxyParagraph}
...
where $y_{rom}$ is the reduced state (or generalized coordinates), and $g$ is the mapping between the two. ~\newline
 If $g$ is linear, then we can write\+: \[ y_{fom} = \phi y_{rom} \] where $\phi$ is a matrix (for the time being, assume it constant). A linear decoder in pressio implements this linear mapping. Since it is linear, the Jacobian of the mapping is\+: \[ \frac{d y_{fom}}{d y_{rom}} = \phi. \]\hypertarget{md_pages_tutorials_tutorial1_autotoc_md40}{}\doxysection{Code}\label{md_pages_tutorials_tutorial1_autotoc_md40}
