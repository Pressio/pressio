...
where $y_{rom}$ is the reduced state, also called generalized coordinates, and $g$ is the mapping between the two. If $g$ is linear, then we can write\+: \[ y_{fom} = \phi y_{rom} \] where $\phi$ is a matrix (for the time being, assume it constant). A linear decoder in pressio implements this linear mapping. Since it is linear, the Jacobian of the mapping is\+: \[ \frac{d y_{fom}}{d y_{rom}} = \phi. \]\hypertarget{md_pages_tutorials_tutorial1_autotoc_md40}{}\doxysection{Code}\label{md_pages_tutorials_tutorial1_autotoc_md40}
...
\DoxyCodeLine{\textcolor{comment}{   creating a linear decoder (or mapping)}}
\DoxyCodeLine{\textcolor{comment}{}}
...
\DoxyCodeLine{\textcolor{comment}{   Here we demonstate how to create a linear decoder object using Eigen types.}}
\DoxyCodeLine{\textcolor{comment}{   For other types already known to pressio (e.g. Trilinos), it would work similarly.}}
\DoxyCodeLine{\textcolor{comment}{   If you work with an arbitrary type currrently unknown to pressio, see tutorial5.cc}}
\DoxyCodeLine{\textcolor{comment}{  */}}
\DoxyCodeLine{}
...
\DoxyCodeLine{\}}
\end{DoxyCode}
\hypertarget{md_pages_tutorials_tutorial1_autotoc_md41}{}\doxysection{Comments}\label{md_pages_tutorials_tutorial1_autotoc_md41}
For other types (e.\+g., Trilinos) already known to pressio (which means pressio knows how to operate on), it would work similarly. If you work with an arbitrary type (i.\+e. one for which pressio does not know how to operate on), see \href{tutorial1udops_8md.html}{\texttt{ this tutorial}}. 