\hypertarget{md_pages_components_rom_decoder_autotoc_md76}{}\doxysection{Overview}\label{md_pages_components_rom_decoder_autotoc_md76}
...
Note that there is no explicit constraint on what the mapping is, it can be anything.\hypertarget{md_pages_components_rom_decoder_autotoc_md77}{}\doxysection{Decoder Concept}\label{md_pages_components_rom_decoder_autotoc_md77}
A valid decoder is any C++ object whose type meets the following API\+:


\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{struct }Decoder}
\DoxyCodeLine{\{}
...
\DoxyCodeLine{    \textcolor{comment}{//  -\/ if romOperand is rank-\/1, it supports the (i) operator to reference the i-\/th element}}
\DoxyCodeLine{    \textcolor{comment}{//  -\/ if romOperand is rank-\/2, it supports the (i,j) operator to reference the i,j-\/th element}}
\DoxyCodeLine{    \textcolor{comment}{// ...}}
\DoxyCodeLine{  \}}
\DoxyCodeLine{}
...
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// return a const reference to the Jacobian matrix}}
\DoxyCodeLine{  \textcolor{keyword}{const} jacobian\_type \& jacobianCRef() \textcolor{keyword}{const};}
\DoxyCodeLine{\};}

\end{DoxyCode}
\hypertarget{md_pages_components_rom_decoder_autotoc_md78}{}\doxysubsection{Requirements}\label{md_pages_components_rom_decoder_autotoc_md78}

\begin{DoxyItemize}
\item {\ttfamily fom\+\_\+state\+\_\+type}\+: must be copy constructible
\item {\ttfamily jacobian\+\_\+matrix\+\_\+type}\+: must be copy constructible
\end{DoxyItemize}\hypertarget{md_pages_components_rom_decoder_autotoc_md79}{}\doxysubsection{Special case\+: linear decoder}\label{md_pages_components_rom_decoder_autotoc_md79}
