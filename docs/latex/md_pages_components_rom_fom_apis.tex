In order to use the {\ttfamily presssio/rom} model reduction functionalities, obviously there needs to be a way to exchange data/information between pressio and your FOM application. To do so, pressio requires you to write an {\itshape adapter class} as a minimally intrusive layer to standardize the way pressio interfaces with any application. As explained below, in general, preparing the adapter should only involve {\itshape exposing} from your applications some operators.\hypertarget{md_pages_components_rom_fom_apis_autotoc_md78}{}\doxysection{Stead API}\label{md_pages_components_rom_fom_apis_autotoc_md78}
Intended for when your FOM application is expressed as \[ \boldsymbol{R}(\boldsymbol{y}; \boldsymbol{\mu}) = 0 \] where $y$ is the full-\/order model (FOM) state, $R$ is the residual todo finish.\hypertarget{md_pages_components_rom_fom_apis_autotoc_md79}{}\doxysubsection{Synopsis}\label{md_pages_components_rom_fom_apis_autotoc_md79}
...
\end{DoxyCode}
\hypertarget{md_pages_components_rom_fom_apis_autotoc_md80}{}\doxysubsection{Usage}\label{md_pages_components_rom_fom_apis_autotoc_md80}
...
~\newline
\hypertarget{md_pages_components_rom_fom_apis_autotoc_md84}{}\doxysection{Continuous-\/time API\+: Velocity and Jacobian action}\label{md_pages_components_rom_fom_apis_autotoc_md84}
This API is intended for any system expressible in {\itshape time-\/continuous} form as above, but you expose both the right-\/hand-\/side of your FOM application as well as the action of the velocity\textquotesingle{}s Jacobian on some operand (more on this later).\hypertarget{md_pages_components_rom_fom_apis_autotoc_md85}{}\doxysubsection{Synopsis}\label{md_pages_components_rom_fom_apis_autotoc_md85}
...
~\newline
\hypertarget{md_pages_components_rom_fom_apis_autotoc_md87}{}\doxysection{Discrete-\/time API}\label{md_pages_components_rom_fom_apis_autotoc_md87}
This API is intended for any system expressible in a discrete-\/time form as \[ \boldsymbol{R}(\boldsymbol{y_{n+1}}, \boldsymbol{y_{n}}, \boldsymbol{y_{n-1}}, ..., t_{n+1}, dt_{n+1}; ...) = \boldsymbol{0} \] where $y$ is the full-\/order model (FOM) state, $t$ is time, and $R$ is the residual. todo finish.\hypertarget{md_pages_components_rom_fom_apis_autotoc_md88}{}\doxysubsection{Synopsis}\label{md_pages_components_rom_fom_apis_autotoc_md88}
...
~\newline
\hypertarget{md_pages_components_rom_fom_apis_autotoc_md90}{}\doxysection{Should you prefer the continuous-\/time or discrete-\/time API?}\label{md_pages_components_rom_fom_apis_autotoc_md90}
