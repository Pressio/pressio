...
Public namespace\+: {\ttfamily pressio\+::ode} \hypertarget{md_pages_components_ode_steppers_implicit_autotoc_md56}{}\doxysection{Overview}\label{md_pages_components_ode_steppers_implicit_autotoc_md56}
...
Here, $y$ is the state, $f$ the velocity, $t$ is time, and $R$ is the residual.\hypertarget{md_pages_components_ode_steppers_implicit_autotoc_md57}{}\doxysection{API for continuous-\/time systems}\label{md_pages_components_ode_steppers_implicit_autotoc_md57}
...
\DoxyCodeLine{\textcolor{keyword}{auto} create\_implicit\_stepper(pressio::ode::StepScheme name,}
\DoxyCodeLine{                             \textcolor{keyword}{const} StateType \& state,}
\DoxyCodeLine{                             ResidualPolicyType \&\& residual\_policy,}
\DoxyCodeLine{                             JacobianPolicyType \&\& jacobian\_policy);}

\end{DoxyCode}
\hypertarget{md_pages_components_ode_steppers_implicit_autotoc_md58}{}\doxysubsection{Parameters}\label{md_pages_components_ode_steppers_implicit_autotoc_md58}

\begin{DoxyItemize}
\item {\ttfamily scheme\+\_\+name}\+:
\end{DoxyItemize}
...
\item {\ttfamily system}\+:
\begin{DoxyItemize}
\item problem instance to query for the velocity $f$ and how to compute it;
\end{DoxyItemize}
\item {\ttfamily residual\+\_\+policy}, {\ttfamily jacobian\+\_\+policy}\+:
\begin{DoxyItemize}
\item policies if you want to use custom ones to compute the discrete operators.
\item the policies encapsulate logic on {\itshape how} to compute the operators
\end{DoxyItemize}
\end{DoxyItemize}
...
\end{DoxyItemize}\hypertarget{md_pages_components_ode_steppers_implicit_autotoc_md59}{}\doxysubsection{Requirements}\label{md_pages_components_ode_steppers_implicit_autotoc_md59}
...
\end{DoxyItemize}\hypertarget{md_pages_components_ode_steppers_implicit_autotoc_md60}{}\doxysubsection{If you use custom policies\+:}\label{md_pages_components_ode_steppers_implicit_autotoc_md60}
...
\item {\ttfamily state}\+:
\begin{DoxyItemize}
\item passed to {\ttfamily call} operator of the policies, contains the prediction at {\ttfamily n+1}.
\end{DoxyItemize}
\item {\ttfamily aux\+States}, {\ttfamily aux\+Rhs}
\begin{DoxyItemize}
\item the types of these you don\textquotesingle{}t need to know
\item contain the needed auxiliary states and RHS evaluations, respectively, needed to compute the operators for a certain scheme. All you need to know about these containers is the following\+:
\end{DoxyItemize}
\end{DoxyItemize}
...
The above factory function returns a stepper instance for the desired scheme. The returned stepper object satisfies the \char`\"{}steppable\char`\"{} concept discussed \href{/Users/fnrizzi/Desktop/work/ROM/gitrepos/pressio/docs/html/md_pages_components_ode_advance.html}{\texttt{ here}}, so one can use the \char`\"{}advancers\char`\"{} functions to step forward. 

~\newline
\hypertarget{md_pages_components_ode_steppers_implicit_autotoc_md61}{}\doxysubsection{What to do after a stepper is created?}\label{md_pages_components_ode_steppers_implicit_autotoc_md61}
...
\end{DoxyCode}
\hypertarget{md_pages_components_ode_steppers_implicit_autotoc_md62}{}\doxysubsection{Required specializations for custom types}\label{md_pages_components_ode_steppers_implicit_autotoc_md62}
...
~\newline
\hypertarget{md_pages_components_ode_steppers_implicit_autotoc_md63}{}\doxysection{API for discrete-\/time systems}\label{md_pages_components_ode_steppers_implicit_autotoc_md63}
todo FINISH
...
\end{DoxyCode}
\hypertarget{md_pages_components_ode_steppers_implicit_autotoc_md64}{}\doxysubsection{Parameters and Requirements}\label{md_pages_components_ode_steppers_implicit_autotoc_md64}

\begin{DoxyItemize}
\item {\ttfamily num\+\_\+states}\+: total number of states you need.
\item {\ttfamily state}\+: your state data
\item {\ttfamily system}\+:
\begin{DoxyItemize}
\item problem instance knowing how to create and compute the residual and Jacobian.
