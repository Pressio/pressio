\hypertarget{md_pages_getstarted_adapter_api_autotoc_md18}{}\doxysection{What is it? Why we need it?}\label{md_pages_getstarted_adapter_api_autotoc_md18}


\begin{DoxyParagraph}{}
An adapter class allows a F\+OM application to expose data via an A\+PI conforming to pressio requirements.
\end{DoxyParagraph}
Recall that pressio assumes your full-\/order model (F\+OM) system to be expressible as \[ \frac{d \boldsymbol{y}}{dt} = \boldsymbol{f}(\boldsymbol{y},t; \boldsymbol{\mu}), \quad \boldsymbol{y}(0;\boldsymbol{\mu}) = \boldsymbol{y}(\boldsymbol{\mu}), \] where $y$ is the F\+OM state and $f(...)$ is the F\+OM velocity, todo finish.

To use the functionalities in pressio, obviously there needs to be a way to exchange data/information between pressio and your F\+OM application. To do so, in pressio we leverage the idea of an {\itshape adapter class} as a layer allowing to standardize the way pressio interfaces with any application.

Schematically, the flow of interfation is shown below\+: 

To facilitate the integration with existing applications, pressio supports adapters with two main types of A\+PI\+:
\begin{DoxyEnumerate}
...
\end{DoxyEnumerate}\hypertarget{md_pages_getstarted_adapter_api_autotoc_md19}{}\doxysection{Continuous-\/time A\+PI}\label{md_pages_getstarted_adapter_api_autotoc_md19}
...
\end{DoxyItemize}\hypertarget{md_pages_getstarted_adapter_api_autotoc_md20}{}\doxysubsection{A\+: velocity only}\label{md_pages_getstarted_adapter_api_autotoc_md20}
...
\end{DoxyParagraph}
\hypertarget{md_pages_getstarted_adapter_api_autotoc_md21}{}\doxysubsection{B\+: velocity and Jacobian action}\label{md_pages_getstarted_adapter_api_autotoc_md21}
...
\end{DoxyParagraph}
\hypertarget{md_pages_getstarted_adapter_api_autotoc_md22}{}\doxysection{Discrete-\/time A\+PI}\label{md_pages_getstarted_adapter_api_autotoc_md22}
