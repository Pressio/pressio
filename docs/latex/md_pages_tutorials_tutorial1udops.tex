...
\end{DoxyParagraph}
\hypertarget{md_pages_tutorials_tutorial1udops_autotoc_md42}{}\doxysection{Context}\label{md_pages_tutorials_tutorial1udops_autotoc_md42}
A key assumption of projection-\/based R\+O\+Ms is to approximate the full-\/order model (F\+OM) state, $y_{fom}$, as\+: \[ y_{fom} = g(y_{rom}) \]

where $y_{rom}$ is the reduced state, also called generalized coordinates, and $g$ is the mapping between the two. If $g$ is linear, then we can write\+: \[ y_{fom} = \phi y_{rom} \] where $\phi$ is a matrix (for the time being, assume it constant). A linear decoder in pressio implements this linear mapping. Since it is linear, the Jacobian of the mapping is\+: \[ \frac{d y_{fom}}{d y_{rom}} = \phi. \]\hypertarget{md_pages_tutorials_tutorial1udops_autotoc_md43}{}\doxysection{Code}\label{md_pages_tutorials_tutorial1udops_autotoc_md43}
...
\DoxyCodeLine{\}}
\end{DoxyCode}
\hypertarget{md_pages_tutorials_tutorial1udops_autotoc_md44}{}\doxysection{The ops struct}\label{md_pages_tutorials_tutorial1udops_autotoc_md44}
