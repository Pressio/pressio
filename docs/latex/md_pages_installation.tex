...
{\ttfamily pressio} is header-\/only, so it does not need to be precompiled and linked to. However, since we use preprocessor directives to conditionally enable/disable code based on target third-\/party libraries, one needs to account for this. See below for the details. 



To use {\ttfamily pressio}, you need a C++14 compiler. \hypertarget{md_pages_installation_autotoc_md159}{}\doxysection{Dependencies}\label{md_pages_installation_autotoc_md159}
Some parts of {\ttfamily pressio} contain code and implementations that are specific to third-\/party libraries (TPLs). An example is {\ttfamily pressio/ops}, which contains kernels specializations for widely-\/used HPC libraries (e.\+g. Trilinos, Kokkos). The main reason for doing this is that we aim, where possible, to alleviate the user from writing custom operations and allow {\ttfamily pressio} to decide when and how to leverage the native libraries\textquotesingle{} operations to obtain the best performance. This should facilitate the integration and use of {\ttfamily pressio} by existing applications. This is a growing capability and we currently only provide built-\/in support to some external HPC libraries (see below). Obviously, these TPLs-\/specific specializations need to be guarded with preprecessor directives, and enabled only if one can access the TPLs.
...
\end{longtabu}
\hypertarget{md_pages_installation_autotoc_md160}{}\doxysubsection{Optional vs Required}\label{md_pages_installation_autotoc_md160}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{preprocessor}{\#include "{}pressio/what\_you\_need.hpp"{}}}
